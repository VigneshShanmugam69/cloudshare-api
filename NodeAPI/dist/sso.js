...
exports.router.post('/generateSmalassertion', async (req, res) => {
  try {
    ...
    const s = await authClient.authenticate({
      ...
      // the authentication was successful, generate a bearer token
      const token = authClient.tokenManager.get('accessToken').accessToken;
      console.log(token);
    })
    res.send(s);
  } catch (error) {
...
exports.router.post('/deleteuser', async (req, res) => {
  ...
  try {
    let f = {
      filter: `profile.firstName eq "${firstName}" and profile.lastName eq "${lastName}"`
    }

    const r = await oktaClient.getUser(userId);
    const rs = await r.update({ f })
    // const a=(await oktaClient.getUser(userId)).delete(userId);
    // await oktaClient.deleteUser(userId);
...
exports.router.post('/final', async (req, res) => {

  const SamlStrategy = require('passport-saml').Strategy;

  passport.use(new SamlStrategy(
    {
      entryPoint: 'http://www.okta.com/exk91fv3nbxpFiO315d7',
      issuer: 'https://us-east-1.signin.aws.amazon.com/platform/saml/d-90677f5a16',
      callbackUrl: 'https://yourapp.example.com/saml/callback',
    },
    (profile, done) => {
      // handle the SAML response here
      done(null, profile);
    }
  ));
})
...
// try {
//   const result= await axios.post(' https://dev-99932483.okta.com/app/amazon_aws_sso/exk91fv3nbxpFiO315d7/sso/saml',(err,data)=>{
//    if(err){
//      return err
//    }
//    return data;
